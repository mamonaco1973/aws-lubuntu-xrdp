# AWS Lubuntu XRDP Cloud Development Environment  

This project provides a **complete cloud-based Linux desktop development environment**
powered by **Lubuntu + XRDP**, **Mini-Active Directory**, and **Amazon EFS** run on AWS.
...
1. **A Custom Lubuntu XRDP AMI (Packer)**
...
3. **Domain-Joined Linux Desktop Server (Terraform)**
   - Deploys Lubuntu XRDP instance using the Packer-built AMI  
...
```bash
develop-vm:~/aws-lubuntu-xrdp$ ./apply.sh
...
- **Networking:**  
  ...
  - Route tables configured to support AD, EFS, and the Lubuntu XRDP servers  
...
5. **Assign a Unique UID Number**  
   ...
   - Run the script located at:  
     ```powershell
     Z:\efs\aws-Lubuntu-xrdp\04-utils\getNextUID.bat
     ```  
