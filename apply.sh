...
#     2. Builds a Packer AMI for Lubuntu XRDP.
#     3. Deploys EC2 servers that join the AD domain.
#
# Goal:
#   Produce a fully configured Lubuntu XRDP server joined to the AD domain.
#
# Key Features:
#   - Runs environment checks before starting the build.
#   - Uses Terraform modules for predictable deployments.
#   - Builds a custom Lubuntu XRDP AMI using Packer.
...
# ------------------------------------------------------------------------------------------------
# Phase 2: Build Lubuntu XRDP AMI with Packer
# ------------------------------------------------------------------------------------------------
# Creates the custom Lubuntu XRDP AMI used by the server module. Networking
...
echo "NOTE: Building Lubuntu XRDP AMI with Packer..."

packer init ./lubuntu_ami.pkr.hcl
...
# ------------------------------------------------------------------------------------------------
# Deploys EC2 servers that rely on the AD domain. This includes the Lubuntu
